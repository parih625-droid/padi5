...
class Product {
  static async create(productData) {
    const { name, description, price, image_url, category_id, stock_quantity = 0, is_amazing_offer = false, sales_count = 0, discount_percentage = 0 } = productData;
    const [result] = await pool.execute(
      'INSERT INTO products (name, description, price, image_url, category_id, stock_quantity, is_amazing_offer, sales_count, discount_percentage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, description, price, image_url, category_id, stock_quantity, is_amazing_offer, sales_count, discount_percentage]
    );
  ...
  static async getAll(filters = {}) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Calculate discounted price if there's a discount
      if (rows[i].discount_percentage && rows[i].discount_percentage > 0) {
        rows[i].discounted_price = rows[i].price * (1 - rows[i].discount_percentage / 100);
      }
    }
