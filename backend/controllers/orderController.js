...
const createOrder = async (req, res) => {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      // Check if it's a rate limiting error and we should retry
      if (error.message && error.message.includes('Too many requests') && retryCount <= maxRetries) {
        ...
        console.log(`🔄 ORDER: Rate limit hit, retrying in ${waitTime/1000} seconds... (attempt ${retryCount}/${maxRetries})`);
...
const initializePayment = async (req, res) => {
  try {
    ...
    const { orderId, gateway = 'mellat' } = req.body;
    const userId = req.user.id;
    console.log('Payment initialization data:', { orderId, gateway, userId });
    
    // Check if this is being rate limited
    if (req.rateLimit) {
      console.log('Rate limit info:', req.rateLimit);
    }
    
    // Only allow Mellat Bank
    if (gateway !== 'mellat') {
      return res.status(400).json({ message: 'Only Mellat Bank is supported' });
    }
...
const verifyPayment = async (req, res) => {
  try {
    ...
    console.log('Verifying payment with gateway:', order.payment_gateway);
    ...
    if (verificationResult.success) {
      console.log('Payment verification successful, updating order status');
      // Update order status
      await Order.updatePaymentInfo(orderId, {
        payment_status: 'paid',
      ...
      console.log('Payment verification failed, updating order status');
      await Order.updatePaymentInfo(orderId, {
